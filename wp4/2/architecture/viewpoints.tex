...
data model (see \Cref{fig:dm_map}) --- however it is important to recognize that these are not isomorphic relationships. For example,
...
Synchronization happens through the HDF5/MPI Vol plugin (see \Cref{frontend: hdf5 + mpi}) but may be implemented as a lightweight  library on top of MPI as well.
...
%Identifying variables that need to be garbage collected is achieved by initializing the metadata for a variable with Epoch=0 and the current creation time.
