...
While data types introduced by computer architectures and software libraries are essential for the data model, they are discussed separately in \Cref{sec: viewpoints/logical/datatypes}.
...
Whatever the sampling regime and dimensionality, values of a variable $V$ will be laid out in storage. In the next section (\ref{subsec-ldm}) we present the logical data model associated with the storage, before giving a mapping of the conceptual data model to storage in section \ref{subsec-mapping}).
...
%A future version of the ESDM will support the internal exploitation of external metadata via plugins, but this version of the ESDM will treat external metadata as atomic and opaque, and serialise and store/replace/delete any such metadata. The internal metadata ...
...
The conceptual data model and the logical data model are described above and summarised in \Cref{fig:cdm_ldm}.
...
data model (see \Cref{fig:dm_map}) --- however, it is vital to recognise that these are not isomorphic relationships. For example,
...
\Cref{fig:datatypes} lists the required interfaces for the compound, array and key-value based data structures.
...
In \Cref{fig:apis ll and high} provides an overview of the available ESDM core interfaces.
...
The synchronisation happens through the HDF5/MPI Vol plugin (see \Cref{frontend: hdf5 + mpi}) but may be implemented as a lightweight library on top of MPI as well.
...
\Cref{fig:epoch semantics} illustrates the ESDM methods to interact with epochs and also shows how the most recent readable view is reconstructed.
...
\paragraph{Events:}
ESDM will publish several regular events. \Cref{fig:semantic notification} lists some candidate event types.
...
\paragraph{Storage System:} Multiple storage systems may be deployed, but for the discussion the abstract representation is sufficient.
...
Most components are shown in \Cref{fig:architecture} are passive, that means functions invoked perform certain operations and return upon completion.
...
In the internal architecture of ESDM, the scheduler is the only exception in using threads.
...
Therefore, we list the requirements in \Cref{chap:requirements} in brief and describe how ESDM addresses them.
...
%* how are Data Models processed internally
%* Abstract: JSON for a description of Data Models info, flexible, but certain fields expected => (more details not yet determined, remains to be seen as implementation continues..)
%* How are certain operations of the Data Models implemented for manipulation of objects within the Data Model
%}
%
%
