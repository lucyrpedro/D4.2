...
For a discussion of the decision process for a mapping of an I/O request to backends refer to \Cref{component: layout} on the layout component.
...
\Cref{fig:esdm scheduler process view} provides an overview of the active and waiting process as the ESDM is handling requests. Notice that Process 1 handles an asynchronous request which allows the application to continue computation, while Process 2 depicts the synchronous case.
